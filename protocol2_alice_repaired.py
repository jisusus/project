...
def run(addr, port, msg):
    ...
    symmetric_key = [random.randint(0, 255) for _ in range(16)]
    logging.debug("symetric_key: {}".format(symmetric_key))
    ...
    logging.info(
        "Encrypted message (AES): {}".format(
            base64.b64encode(encrypted_msg).decode()
        )
    )
